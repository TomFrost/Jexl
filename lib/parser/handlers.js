...
exports.lambdaExpression = function(ast) {
	...
	} else if (this._cursor.type !== 'LambdaExpression')
		throw new Error();
	this._cursor.right = ast;
};
...
exports.lambdaExpressionStart = function() {
	...
	this._cursor = this._tree;
	this._incompletable = true;
}

/**
 * Handles assignment of a transform to the Evaluator.
 * @param {{type: <string>}} token A token object
 */
...
exports.argName = function(ast) {
	if ((ast.type !== 'Identifier') && (ast.type !== 'identifier'))
		throw new Error("HUH?");
	this._cursor.argNames.push(ast.value);
};
...
/**
 * Helper function which returns an array of argument names of a referrant
 * subexpression tree.
 * @param  {{type: <string>}} ref A referrant subexpression tree.
 */
var _fillArgNames = function(ref) {
	var argNames = [];
	while (ref) {
		if (ref.type !== 'Identifier')
			throw new Error('Arguments must be identifiers.');
