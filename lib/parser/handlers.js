...
exports.argNames = function(token) {
	this._placeBeforeCursor({
		type: 'ArgumentNames',
		value: [this._cursor.value].concat([token.value])
	});
};
...
exports.identifier = function(token) {
	var prevIdent = this._cursor && this._cursor.type === 'Identifier',
		node = {
			type: 'Identifier',
			value: token.value,
			lineNo: token.lineNo
		};
	this._prevIdent = this._prevIdent && prevIdent;
	if (this._nextIdentEncapsulate || this._prevIdent) {
