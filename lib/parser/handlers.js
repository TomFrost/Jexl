...
/**
 * Handles a passing of context to a new expression by placing a PassContext
 * node before the cursor.
 * @private
 */
exports.passContext = function() {
	this._placeBeforeCursor({
		type: 'PassContext',
		subject: this._cursor
	});
...
/**
 * Handles shoving a variable to a context to a new expression by placing a
 * ShoveContextAssignment node before the cursor with the name of the token.
 * @param {{type: <string>}} token A token object
 * @private
 */
exports.shoveContextAssignment = function(token) {
	this._placeBeforeCursor({
		type: 'ShoveContextAssignment',
		name: token.value,
		subject: this._cursor
	});
};

exports.pullContextAssignmentStart = function(token) {
	this._tree = {
		type: 'PullContextAssignment',
		name: this._tree.value
	};
	this._cursor = this._tree;
};

exports.pullContextAssignmentEnd = function(ast) {
	this._cursor.subject = ast;
};
 ...
 * @private
 */
exports.lambdaExpressionStart = function(ast) {
	var argNames = [];
	var ref;
	if (this._tree !== null) {
		ref = this._tree;
...
exports.lambdaExpressionEnd = function(ast) {
	...
	delete this._cursor.argNames;
	this._cursor.lambda =  function() {
		var args = Array.prototype.slice.call(arguments);
		var context = {};
		for (j = 0, len = argNames.length; j < len; j++) {
			context[argNames[j]] = args[j];
		}
		var evaluator = new Evaluator(self._grammar, self._transforms, context);
