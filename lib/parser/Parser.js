...
function Parser(grammar, prefix, stopMap, state) {
	...
	this._state = state || 'expectFirstOperand';
}
 ...
 *      the expression, indicating that the expression is incomplete
 */
Parser.prototype._complete = function(isWholeExp) {
	var cur = this._cursor;
	if (cur && (!states[this._state].completable
		|| (isWholeExp && cur._incompletable))) {
...
Parser.prototype._endSubExpression = function() {
	states[this._state].subHandler.call(this, this._subParser._complete());
...
/**
 * Indicates a terminal cursor with this node as the top is not complete.
 * @param {{type: <string>}} node A node of the AST
 * @private
 */
Parser.prototype._markIncompletable = function(node) {
	Object.defineProperty(node, '_incompletable', {
		value: true,
		writable: true
	});
};
...
Parser.prototype._startSubExpression = function(exprStr) {
	var state = states[this._state],
		...
		subState = state.initial ? 'expectInitialOperand' : 'expectOperand';
	if (!endStates) {
		this._parentStop = true;
		endStates = this._stopMap;
	}
	this._subParser = new Parser(this._grammar, exprStr, endStates, subState);
};
