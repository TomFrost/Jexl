 ...
 * @constructor
 */
function Parser(grammar, prefix, stopMap, breakMap) {
	this._grammar = grammar;
	this._state = 'expectFirstOperand';
	this._tree = null;
...
Parser.prototype.addToken = function(token) {
	...
	var state = states[this._state],
		...
		tokenTypes = null;
	if (this._appearsMap) {
		Object.keys(this._appearsMap).forEach(function(tokenType) {
			appearsAsType = self._appearsMap[tokenType];
			if (appearsAsType) {
				tokenTypes = tokenTypes || {};
				tokenTypes[tokenType] = tokenTypes[appearsAsType];
			}
		});
	}
	if (state.tokenTypes) {
		Object.keys(state.tokenTypes).forEach(function(tokenType) {
			tokenTypes = tokenTypes || {};
			tokenTypes[tokenType] = state.tokenTypes[tokenType];
		});
	}
	...
	else if (tokenTypes[token.type]) {
		var typeOpts = tokenTypes[token.type],
...
Parser.prototype._startSubExpression = function(exprStr) {
	...
	var appearsAs = states[this._state].appearsAs;
	if (!endStates) {
	...
	if (appearsAs)
		this._subParser._appearsMap = appearsAs;
};
