...
Jexl.prototype._eval = function(exp, context) {
	...
	return Promise.resolve().then(function() {
		var tokenizedLines = self._getLexer().tokenizeLines(exp);
		var parsers = tokenizedLines.map(function(tokenizedLine) {
			parser = new Parser(grammar),
			parser.addTokens(tokenizedLine);
			completeParser = parser.complete();
			return completeParser;
		});
		var runningPromise = null;
		parsers.forEach(function(parser) {
			if (!runningPromise)
				runningPromise = evaluator.eval(parser);
			else {
				runningPromise = runningPromise.then(function() {
					return evaluator.eval(parser);
				});
			}
		});
