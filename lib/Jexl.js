...
class Jexl {
   ...
   * @param {Object} [context] A mapping of variables to v alues, which will be
   *      made accessible to the Jexl expression when evaluating it
   * @returns {Promise<*>} resolves with the result of the evaluation.
   */
  eval (expression, context = {}) {
    const { parser, evaluator } = this._setupEval(expression, context)
   ...
   */
  syncEval (expression, context = {}) {
    const { parser, evaluator } = this._setupEval(expression, context)
    parser.addTokens(this._getLexer().tokenize(expression))
    const ast = parser.complete()
    return evaluator.syncEval(ast)
  }
  ...
  /**
   * Set up World for syncEval() and eval().
   * Grammar, Parser and Evaluator get instantiated and returned in an object.
   * @param {string} expression The Jexl expression to be evaluated
   * @param {Object} [context] A mapping of variables to v alues, which will be
   *      made accessible to the Jexl expression when evaluating it
   * @returns {{parser:{},evaluator:{}}} resolves with the result of the evaluation.
   */
  _setupEval (expression, context) {
