...
Jexl.prototype.compile = function(exp) {
	return this._eval(this._compile(exp, true));
};
...
Jexl.prototype.eval = function(exp, context, cb) {
	return this._eval(null, this._compile.bind(this, exp, true))(context, cb);
};
...
Jexl.prototype.stream = function(exp, context) {
	return this._eval(null, this._compile.bind(this, exp, false))(context);
};
 ...
 * @private
 */
Jexl.prototype._compile = function(exp, consume) {
	var grammar = this._getGrammar(),
		transforms = this._transforms,
		tokenizedLines = this._getLexer().tokenizeLines(exp.trim());
	try {
		var completeLines = Parser.completeLines(grammar, tokenizedLines);
	} catch (e) {
		throw new Error(e.message + " assigned multiple times in expression: " + exp);
	}
	var metaEvaluator = new Evaluator.Meta(grammar, transforms, completeLines, consume);
