...
Jexl.prototype.compile = function(exp) {
	return this._eval(this._compile(exp));
};
...
Jexl.prototype.eval = function(exp, context, cb) {
	return this._eval(null, this._compile.bind(this, exp))(context, cb);
};

/**
 * Streams the results of evaluation of a Jexl string within an optional context.
 * @param {string} exp The Jexl expression to be evaluated
 * @param {Object} [context] A mapping of variables to values, which will be
 *      made accessible to the Jexl expression when evaluating it
 * @returns {EventEmitter<*>} emits 'data' events with results, 'error' events
 *      with errors, and an 'end' event on completion.
 */
Jexl.prototype.stream = function(exp, context) {
	return this._eval(null, this._compile.bind(this, exp, true))(context);
};
 ...
 * @private
*/
Jexl.prototype._eval = function(contextEval, boundCompiler) {
...
Jexl.prototype._compile = function(exp, lazy) {
			...
			.map(function(tokenizedLine) {
				...
				if (parser._assignment) {
					if (assigned[parser._assignment]) {
						throw new Error("" + parser._assignment + " assigned to multiple"
	...
	var metaEvaluator = new Evaluator.Meta(grammar, transforms, completeLines, lazy);
	return metaEvaluator.eval.bind(metaEvaluator);
};
