...
Jexl.prototype.compile = function(exp) {
	return this._genEval(this._compile(exp));
};
 ...
 *      the callback.
 */
Jexl.prototype.eval = function(exp, context, cb) {
	return this._genEval(null, this._compile.bind(this, exp))(context, cb);
};

Jexl.prototype.stream = function(exp, context) {
	return this._genEval(null, this._compile.bind(this, exp, true))(context);
};
...
/**
 * Returns a function to evaluate within an optional context.
 * @param {Function} contextEval A function to evaluate within a context
 * @param {Function} boundCompiler A function which returns a contextEval
 *      when none is specified
 * @private
*/
Jexl.prototype._genEval = function(contextEval, boundCompiler) {
 ...
 * @private
 */
Jexl.prototype._compile = function(exp, lazy) {
	var grammar = this._getGrammar(),
		transforms = this._transforms,
		assigned = {},
		completeLines = this._getLexer().tokenizeLines(exp.trim())
			.map(function(tokenizedLine) {
				var parser = new Parser(grammar);
				parser.addTokens(tokenizedLine);
				line = parser.complete(true);
				if (parser._assignment) {
					if (assigned[parser._assignment]) {
						throw new Error("Identifier " + line.name + " assigned to multiple"
							+ " times in expression: " + exp);
					}
					assigned[parser._assignment] = true;
				}
				return line;
			});
	return this._eval.bind(this, grammar, transforms, completeLines, lazy);
};
 ...
 * @private
 */
Jexl.prototype._eval = function(grammar, transforms, lines, lazy, context) {
	var evaluator = new Evaluator(grammar, transforms, context);
	promise = evaluator.evalLines(lines, lazy).then(function(arr) {
		return arr.pop();
	});
	if (!lazy)
		return promise;
	var relay = new EventEmitter;
	promise.then(function(emitter) {
		['data', 'error', 'end'].forEach(function(event) {
			emitter.on(event, relay.emit.bind(relay, event));
		});
	});
	return relay;
};
