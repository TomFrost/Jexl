...
var numericRegex = /^-?(?:(?:[0-9]*\.[0-9]+)|[0-9]+)$/,
	...
	minusNegatesAfter = ['binaryOp', 'unaryOp', 'openParen', 'openBracket',
		'question', 'colon', 'semicolon', 'shove', 'push', 'lambdaExpression'];
...
/**
 * Converts a Jexl string into an array of tokenized lines.  Each tokenized line
 * is an array of tokens. A string is split by the newline character and each
 * line is then tokenized. Lines consisting of only space characters are ignored.
 * @param {string} str The Jexl string to be converted to tokenized lines
 * @returns {Array<{type, value, raw}>} an array of arrays of token objects.
 * @throws {Error} if the provided string contains an invalid token.
 */
Lexer.prototype.tokenizeLines = function(str) {
	...
	str.split('\n').forEach(function(line) {
		if (!self._isWhitespace(line))
			tokenizedLines.push(self.tokenize(line));
	});
