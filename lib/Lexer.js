...
var numericRegex = /^-?(?:(?:[0-9]*\.[0-9]+)|[0-9]+)$/,
	...
	minusNegatesAfter = ['binaryOp', 'unaryOp', 'groupStart', 'drillStart',
		'transformKeyValSep'];
 ...
 */
Lexer.getElements = function(str) {
	var regex = Lexer._getSplitRegex();
 ...
 */
Lexer.getTokens = function(elements) {
	var tokens = [],
		negate = false;
	for (var i = 0; i < elements.length; i++) {
		if (Lexer._isWhitespace(elements[i])) {
			...
			tokens.push(Lexer._createToken(elements[i]));
		}
	}
	// Catch a - at the end of the string. Let the parser handle that issue.
	if (negate)
		tokens.push(Lexer._createToken('-'));
	return tokens;
};
...
Lexer.tokenize = function(str) {
	var elements = Lexer.getElements(str);
	return Lexer.getTokens(elements);
};
 ...
 * @private
 */
Lexer._createToken = function(element) {
	var eName = eNames[element],
		token = {
			type: 'literal',
			value: element,
			raw: element
	...
	if (element[0] == '"' || element[0] == "'")
		...
		token.type = Grammar.elements[eName].type || eName;
		token.name = eName;
	}
 ...
 * @private
 */
Lexer._escapeRegExp = function(str) {
 ...
 * @private
 */
Lexer._getSplitRegex = function() {
	if (!splitRegex) {
		var elemArray = [];
		// Construct an array of each element's 'str' property
		for (var key in Grammar.elements) {
			if (Grammar.elements.hasOwnProperty(key)) {
				elemArray.push(Grammar.elements[key].str);
			}
		}
		// Sort them by most characters to least, then regex escape each
		elemArray = elemArray.sort(function(a ,b) {
			return b.length - a.length;
		}).map(function(elem) {
			return Lexer._escapeRegExp(elem);
		});
		splitRegex = new RegExp('(' + nativeRegexElems.join('|') + '|' +
			elemArray.join('|') + ')');
	}
	return splitRegex;
};
 ...
 * @private
 */
Lexer._isNegative = function(tokens) {
	if (!tokens.length)
		return true;
 ...
 * @private
 */
Lexer._isWhitespace = function(str) {
