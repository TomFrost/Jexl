...
exports.AssignmentExpression = function(ast) {
	...
	return new Promise(function(resolve, reject) {
		var assignExpression = function() {
			...
			Promise.resolve(self.eval(ast.right)).then(function(right) {
				return self._grammar[ast.operator].eval(self._context[ast.name], right);
			}).then(function(res) {
				var newContext = {};
		...
		var getIdentifier = function() {
			if (self._extantLines[0] < ast.lineNo)
				...
				Promise.resolve(ast.relative ? self._relContext[ast.value] :
					self._context[ast.value]).then(resolve).catch(reject);
			}
		};
		getIdentifier();
	});
