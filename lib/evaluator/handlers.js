...
exports.CollectExpression = function(ast) {
	...
	return this.eval(ast.subject).then(function(subject) {
		return self._collect(subject, ast.expr);
	});
};
	...
	return new Promise(function(resolve, reject) {
		var getIdentifier = function() {
			if (self._extantLines[0] < ast.lineNo)
				...
				resolve((ast.relative ? self._relContext : self._context)[ast.value]);
			else {
...
exports.TransformAssignmentExpression = function(ast) {
	...
	transforms[ast.name] = function() {
		var args = arguments,
			context = {};
		ast.args.forEach(function(name, i) {
			context[name] = args[i];
		});
		...
		return evaluator.eval(ast.right);
	};
