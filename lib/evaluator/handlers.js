 ...
 * Evaluates a Transform node by applying a function from the transforms map
 * to the subject value.
 * @param {{type: 'Extractor', name: <string>, subject: {}}} ast An
 *      expression tree with an Extractor as the top node
 * @returns {Promise<*>|*} the value of the transformation, or a Promise that
 *      will resolve with the transformed value.
 * @private
 */
exports.Extractor = function(ast) {
	var extractor = this._extractors[ast.name];
	if (!extractor)
		throw new Error("Extractor '" + ast.name + "' is not defined.");
	return Promise.all([
		this.eval(ast.subject),
		this.evalArray(ast.args || [])
	]).then(function(arr) {
