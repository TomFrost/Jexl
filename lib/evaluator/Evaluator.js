...
Evaluator.prototype.eval = function(ast) {
	return Promise.resolve()
		.then(this.evalLazy.bind(this, ast))
		.then(utils.consume);
};
...
Evaluator.prototype.evalLines = function(lines) {
	var result = lines.map(this.evalLazy, this).pop();
	this._deferred.resolve();
	return utils.consume(result);
};
...
Evaluator.prototype._subProc = function(ast, subProc) {
	var self = this,
		subject = this.evalLazy(ast.subject);
	var fn = function(obj) {
	...
	if (subject instanceof EventEmitter)
		return fn(subject);
	return utils.consume(subject).then(fn);
};
...
MetaEvaluator.prototype.evalArray = function(arr) {
	...
	return this._consume ? utils.consume(results) : utils.stream(results);
};
