...
class Evaluator {
  ...
  /**
   * Synchronously evaluates an expression tree within the configured context.
   * @param {{}} ast An expression tree object
   * @returns {*} the resulting value of the expression.
   */
  syncEval (ast) {
    const handlerFunction = handlers['sync' + ast.type]
    if (handlerFunction) {
      return handlerFunction.call(this, ast)
    }
    return handlers[ast.type].call(this, ast)
  }
  ...
  /**
   * Synchronously evaluates each expression within an array, and delivers the
   * response as an array with the resulting values at the same indexes as their
   * originating expressions.
   * @param {Array<string>} arr An array of expression strings to be evaluated
   * @returns {Array<{}>} with the result array
   */
  syncEvalArray (arr) {
    return arr.map((elem) => {
      return this.syncEval(elem)
    })
  }
  ...
  /**
   * Synchronously evaluates each expression within a map, and delivers the
   * response as a map with the same keys, but with the evaluated result for each
   * as their value.
   * @param {{}} map A map of expression names to expression trees to be
   *      evaluated
   * @returns {{}} with the result map.
   */
  syncEvalMap (map) {
    const keys = Object.keys(map)
    const result = {}
    const asts = keys.map((key) => {
      return this.syncEval(map[key])
    })
    asts.forEach((val, idx) => {
      result[keys[idx]] = val
    })
    return result
  }
  ...
  _filterRelative (subject, expr) {
    ...
    subject.forEach((elem) => {
      const evalInst = new Evaluator(this._grammar, this._transforms, this._context, elem)
  ...
  /**
   * Synchronously applies a filter expression with relative identifier elements to a subject.
   * The intent is for the subject to be an array of subjects that will be
   * individually used as the relative context against the provided expression
   * tree. Only the elements whose expressions result in a truthy value will be
   * included in the resulting array.
   ...
   *      tree evaluates to a truthy result, then the value will be included in
   *      the returned array otherwise, it will be eliminated.
   * @returns {Array} with an array of values that passed the
   *      expression filter.
   * @private
   */
  _syncFilterRelative (subject, expr) {
    const evalResults = []
    if (!Array.isArray(subject)) {
      subject = [subject]
    }
    subject.forEach((elem) => {
      const evalInst = new Evaluator(this._grammar, this._transforms, this._context, elem)
      evalResults.push(evalInst.syncEval(expr))
    })

    const results = []
    evalResults.forEach((value, idx) => {
      if (value) {
        results.push(subject[idx])
      }
    })
  ...
  /**
   * Synchronously applies a static filter expression to a subject value.  If the filter
   * expression evaluates to boolean true, the subject is returned if false,
   * undefined.
   *
   * For any other resulting value of the expression, this function will attempt
   ...
   * @private
   */
  _syncFilterStatic (subject, expr) {
    const res = this.syncEval(expr)
    if (typeof res === 'boolean') {
      return res ? subject : undefined
    }
    return subject[res]
