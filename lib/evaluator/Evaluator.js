...
/**
 * Applies a collect subexpression to the values of a subject. In the context
 * of the subexpression the '@' identifier refers to the value and the '#'
 * identifier refers to the numeric index if the subject is an array and refers
 * to the string key otherwise.
 *
 * If the subject is not an object of values, it will be converted to a single-
 * element array before running the collect and the value of the single result
 * is returned.
 ...
 * @private
 */
Evaluator.prototype._collect = function(subject, expr) {
	var self = this,
		isObject = (typeof subject === 'object') && !!subject,
		isArray = Array.isArray(subject),
		obj = isObject ? subject : [subject];
	var promises = Object.keys(obj).map(function(key, idx) {
		var id = isArray ? idx : key,
			context = {'#': id, '@': obj[key], '~': breakIndicator};
		Object.keys(self._context).forEach(function(k) {
			if (!(k in context))
				context[k] = self._context[k];
		});
		var evalInst = new Evaluator(self._grammar, self._transforms, context);
		return Promise.all([id, evalInst.eval(expr)]);
	});
	return Promise.all(promises).then(function(tuples) {
		if (!isObject)
			return (tuples[0][1] !== breakIndicator ? tuples[0][1] : undefined);
		var results = isArray ? [] : {};
		tuples.forEach(function(tuple) {
			if (tuple[1] !== breakIndicator)
