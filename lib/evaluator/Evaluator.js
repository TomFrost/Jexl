 ...
 * @private
 */
Evaluator.prototype._iter = function(subject, expr, collect) {
	var self = this,
		isObject = (typeof subject === 'object') && !!subject,
		obj = isObject ? subject : [subject],
		isArray = Array.isArray(obj),
		keys = Object.keys(obj);
	var genPromise = function(key, idx) {
	...
	if (collect) {
		return Promise.all(keys.map(genPromise)).then(function(values) {
			var results = isArray ? [] : {};
		...
		return new Promise(function(resolve, reject) {
			var iterFind = function(idx) {
				if (idx >= keys.length)
					return resolve(undefined);
				genPromise(keys[idx], idx).then(function(val) {
					if (val === omit || !val)
						return iterFind(idx + 1);
					resolve(val);
				});
			};
			iterFind(0);
