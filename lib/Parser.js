...
var states = {
	expectOperand: {
		tokenTypes: {
			...
			unaryOp: {},
			groupStart: {toState: 'subExpression'},
			identTraverse: {toState: 'identTraverse'}
		}
	},
	expectBinOp: {
		tokenTypes: {
			binaryOp: {toState: 'expectOperand'},
			transformStart: {toState: 'expectTransform'}
		},
		completable: true
	},
	expectTransform: {
	...
	expectArgKey: {
		tokenTypes: {
			identifier: {
				...
				handler: '_handle_argKey'
			},
			transformArgsEnd: {toState: 'postTransform'}
		}
	},
	expectKeyValSep: {
		tokenTypes: {
			transformKeyValSep: {toState: 'argVal'}
		}
	},
	postTransform: {
		tokenTypes: {
			transformArgsStart: {toState: 'expectArgKey'},
			binaryOp: {toState: 'expectOperand'},
			identTraverse: {toState: 'identTraverse'},
			filterStart: {toState: 'filter'},
			transformStart: {toState: 'expectTransform'}
		},
		completable: true
	},
	...
	filter: {
		...
		stateOnTrue: 'identifier'
	},
	subExpression: {
		handler: '_handle_subExpression',
		stateOnTrue: 'expectBinOp'
	},
	argVal: {
		handler: '_handle_argVal'
	}
};
...
Parser.prototype.addToken = function(token) {
	...
	if (state.handler) {
		var result = this[state.handler].call(this, token);
		...
		var typeOpts = state.tokenTypes[token.type],
			handleFunc = this['_handle_' + token.type];
		if (typeOpts.handler)
			handleFunc = this[typeOpts.handler];
		if (handleFunc)
...
Parser.prototype._handle_argVal = function(token) {
	if (token.type == 'transformArgsStart')
		this._subTracker++;
	else if (token.type == 'transformArgsEnd')
		this._subTracker--;
	if (this._subTracker == -1 || (this._subTracker == 0 &&
			token.type == 'transformKeyValPairSep')) {
		this._cursor.args[this._curArgKey] = this._subParser.complete();
		if (token.type == 'transformKeyValPairSep')
...
Parser.prototype._handle_binaryOp = function(token) {
	var weight = Grammar.elements[token.name].weight || 0,
		parent = this._cursor._parent;
	while (parent && parent.operator &&
			Grammar.elements[parent.operator].weight >= weight) {
		this._cursor = parent;
...
Parser.prototype._handle_filter = function(token) {
	if (token.type == 'filterStart')
		this._subTracker++;
	else if (token.type == 'filterEnd')
		this._subTracker--;
 ...
 * @private
 */
Parser.prototype._handle_identTraverse = function(token) {
 ...
 * @private
 */
Parser.prototype._handle_transformKeyValSep = function(token) {
