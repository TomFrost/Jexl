...
Parser.prototype.addToken = function(token) {
	...
	if (state.subHandler) {
		...
		if (lastBalance === this._subMarker && state.endStates[token.type]) {
			var subAST = this._subParser.complete();
			this[handlerPrefix + state.subHandler].call(this, subAST, token);
 ...
 * @param @param {{type: <string>}} ast The subexpression tree
 * @private
 */
