...
const { expr } = jexl
const hello = expr`"Hello " + name` // Also: jexl.compile('"Hello " + place')
danger.evalSync({ place: 'world' }) // Hello world
danger.evalSync({ place: 'WORLD!!!' }) // Hello WORLD!!! (Doesn't recompile the expression!)
```
